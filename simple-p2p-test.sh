...
# Упрощенный тест P2P соединения без API регистрации
# Автор: DevOps Team
# Дата: 2025-09-20
...
log_info "🚀 Упрощенное тестирование P2P соединения CloudBridge Relay"
log_info "=========================================================="
...
# Проверка токена для удаленной машины
...
if nc -u -w 3 "$RELAY_HOST" "$QUIC_PORT" < /dev/null 2>/dev/null; then
    ...
    log_warning "Relay сервер $RELAY_HOST:$QUIC_PORT недоступен (может быть нормально)"
fi

# Создание упрощенной конфигурации для удаленной машины
log_info "Создание упрощенной конфигурации для удаленной машины..."
REMOTE_CONFIG=$(cat << 'EOF'
# CloudBridge Client Remote Test Configuration (Simplified)
relay:
  host: "b1.2gc.space"
  ports:
    quic: 5553
...
log_info "Мониторинг соединения в течение 60 секунд..."
log_info "Проверьте логи клиентов для подтверждения P2P соединения"
echo

# Показ логов каждые 10 секунд
for i in {1..6}; do
    log_info "=== Логи через $((i*10)) секунд ==="
    
    log_info "Логи локального клиента:"
    tail -5 local-client.log 2>/dev/null || log_warning "Логи локального клиента недоступны"
    
    echo
    log_info "Логи удаленного клиента:"
    ssh -i "$REMOTE_KEY" -o StrictHostKeyChecking=no "$REMOTE_USER@$REMOTE_HOST" "tail -5 remote-client.log 2>/dev/null" || log_warning "Логи удаленного клиента недоступны"
...
echo
log_success "Упрощенное тестирование P2P соединения завершено! 🚀"
